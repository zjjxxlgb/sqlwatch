...
static my_bool opt_debugging= 0, opt_external_locking= 0, opt_console= 1;
...
bool opt_error_log= 0; //IF_WIN(1,0);
...
int init_common_variables()
{
  ...
  mysql_data_home=mysql_real_data_home; //add by xuliang 20151222
...
a file name for --log-bin-index option", opt_binlog_index_name);
 ...
 // if (!DEFAULT_ERRMSGS[0][0]) delete by xuliang 20151222
//    unireg_abort(1);
...
struct my_option my_long_options[]=
{
  ...
  */
  ...
  {"console", OPT_CONSOLE, "Write error output on screen; don't remove the console window on windows.",
   &opt_console, &opt_console, 0, GET_BOOL, NO_ARG, 1, 0, 0,
   0, 0, 0},
